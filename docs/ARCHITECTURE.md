...
**Lifecycle source of truth (the ledger → derived projections).**
- `pipeline/state/videos.json` (schema `videos.schema.json`) is the **only writable lifecycle state**:
  one record per `publish.json`, owning that video's status, `youtube_video_id`, lesson obligation and
  analytics schedule. `ideas.json` status/`produced_video_id`, each video's `publish.json` status and
  `produced_subjects.json` are **derived projections** regenerated by `pipeline/state/reconcile.mjs` —
  **one-way** (ledger → derived) and **forward-only** (nothing ever demotes). The reconciler is
  pure-core + fs edge like the rest of §12: `ingestPublish` / `project*` / `lessonOwed` are io-free and
  each reports `changed`, so "nothing to do" is cheap enough to run on every turn-end. A publish.json's
  **presence** is the owner-approval signal (Gate ③); its own `status` field can only raise the ledger,
  never cap it. The ledger governs `live_from` (`"020"`) onward; ≤019 carries a closed backfill stamp.
- `.claude/hooks/publish-close.mjs` (Stop hook) runs `reconcile --fix` silently, then blocks only on a
  `lesson.state: "pending"` record — the one thing a machine can't produce (D-061 → D-062).
  `docs/CHANNEL_MAP.md` stays human prose; code owns only its machine mirror.
