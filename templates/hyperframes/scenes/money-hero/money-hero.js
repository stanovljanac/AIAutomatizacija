/* money-hero — bold cinematic GOLD-COIN money-storm hook (009; fork of hook-prism, 16:9 + 9:16).
 * Same deterministic seek-driven Three.js infra as hook-prism (seeded PRNG, hf-seek render, pinned
 * pixel ratio) — only the particle SPRITE (now a glowing gold coin) + palette are money-themed, so
 * the opener SHOWS the subject (a storm of cash) instead of a generic aurora ([[no-title-card-scenes]]).
 *
 *
...
var S = HF.scene({ id: "money-hero", width: 1920, height: 1080, frames: 189 });
...
var title = typeof props.title === "string" && props.title.trim() ? props.title.trim() : "make me money";
...
import(THREE_URL)
  ...
  .catch(function (err) {
    ...
    if (window.console && console.warn) console.warn("[money-hero] 3D layer unavailable:", err);
  });
...
function initThree(THREE) {
  ...
  // GOLD-COIN sprite texture (canvas; built once, deterministically). A glowing gold disc with a
  // bright rim ring reads as a coin under additive blending — near the camera it's a clear coin,
  // far away a gold spark. White is multiplied by the per-particle gold vertex color, so the disc
  // takes the gold tint; the rim stays brighter for the coin edge. The "$" is drawn brightest so it
  // adds the most light (legible on the big near-camera coins). Money-storm read, fully frame-pure.
  function makeCoinTexture() {
    var c = document.createElement("canvas");
    c.width = 64;
    c.height = 64;
    var ctx = c.getContext("2d");
    // disc body: bright core → gold falloff → transparent edge
    var grd = ctx.createRadialGradient(32, 32, 0, 32, 32, 30);
    grd.addColorStop(0.0, "rgba(255,255,255,1)");
    grd.addColorStop(0.45, "rgba(255,245,210,0.95)");
    grd.addColorStop(0.78, "rgba(255,210,120,0.45)");
    grd.addColorStop(1.0, "rgba(255,190,80,0)");
    ctx.fillStyle = grd;
    ctx.beginPath();
    ctx.arc(32, 32, 30, 0, Math.PI * 2);
    ctx.fill();
    // coin rim ring (bright edge)
    ctx.strokeStyle = "rgba(255,240,190,0.95)";
    ctx.lineWidth = 3.5;
    ctx.beginPath();
    ctx.arc(32, 32, 22, 0, Math.PI * 2);
    ctx.stroke();
    // a bright "$" so big coins read unmistakably as money
